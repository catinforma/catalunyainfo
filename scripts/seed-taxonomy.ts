/**
 * Seeds the taxonomy: sections, categories and the controlled tag vocabulary.
 *
 * This is structure, not content. It creates the shelves; the editorial team
 * puts things on them. Nothing here is an article, a news item or a guide, and
 * the script creates no entries of any kind.
 *
...
async function main() {
  ...
  console.log(
    `Taxonomy seeded: ${CATEGORIES.length} categories, ${TAGS.length} tags. No content was created.`,
  );
  process.exit(0);
}
